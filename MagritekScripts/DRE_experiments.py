...
# perform DRE in-situ
def shot(distortion, set_as_string, model_set, offset_value, ray_results):
...
for d in random_distortions:
    pred, lw_init, lw_shimmed = shot(d, 'poc', ENSEMBLE, ray_results=my_arg.base_models, offset_value=my_arg.offset_value)
    success, criterion_after = checkup(pred, d, offset_value=my_arg.offset_value)
